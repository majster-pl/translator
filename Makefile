...
# 1) run make once to create and update the po/translator-app.pot
# 2) copy the template file and set the name to the language you want to
#    translate to:   cp po/translator-app.pot po/en.po
# 3) edit the po file
...
APP_ID=translator-app.majster-pl
...
po/translator-app.pot: $(QMLJS_FILES)
	mkdir -p $(CURDIR)/po && xgettext -o po/translator-app.pot --qt --c++ --add-comments=TRANSLATORS --keyword=tr --keyword=tr:1,2 $(QMLJS_FILES) --from-code=UTF-8
